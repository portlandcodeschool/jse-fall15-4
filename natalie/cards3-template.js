...
makeCard.rank = function(id){
  return Math.floor(id/4)+1;
}

makeCard.suit = function(id){
  return ((id%4)+1);
}

makeCard.color = function(id){
  var theSuit=this.suit(id);
  return theSuit && ((theSuit<3)? "red" : "black");
}

makeCard.cardName = function(id){
  var theRank = makeCard.rank(id);
  var theSuit = makeCard.suit(id);
...
function makeCard(id) {
  ...
  var temp = {
    id: id,
    rank: makeCard.rank(id),
    suit: makeCard.suit(id),
    color: makeCard.color(id),
    name: makeCard.cardName(id),
  }
  return temp;
}
//-----------------------------
...
makeCard.isCard = function(obj) { // --> true,false
  ...
  if (!(makeCard.isValid(obj.id, 0, 51)))
    return false;
  if (!(makeCard.rank(obj.id) === obj.rank && makeCard.suit(obj.id) === obj.suit
        && makeCard.color(obj.id) === obj.color && makeCard.cardName(obj.id) === obj.name))
    return false;
  return true;
}
